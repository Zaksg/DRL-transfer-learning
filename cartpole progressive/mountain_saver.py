...
env._max_episode_steps = 7000  # 4000
env.seed(1)
np.random.seed(1)
...
class PolicyNetwork:
    def __init__(self, state_size, action_size, name='policy_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W1 = tf.get_variable("W1_2", [self.state_size, 12],
                                      initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b1 = tf.get_variable("b1_2", [12], initializer=tf.zeros_initializer())
            self.W2 = tf.get_variable("W2_2", [12, 12], initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b2 = tf.get_variable("b2_2", [12], initializer=tf.zeros_initializer())
            self.W3 = tf.get_variable("W3_2", [12, self.action_size],
                                      initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b3 = tf.get_variable("b3_2", [self.action_size], initializer=tf.zeros_initializer())
...
class ValueNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='value_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W1 = tf.get_variable("W1_2", [self.state_size, 20],
                                      initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b1 = tf.get_variable("b1_2", [20], initializer=tf.zeros_initializer())
            self.W2 = tf.get_variable("W2_2", [20, 20], initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b2 = tf.get_variable("b2_2", [20], initializer=tf.zeros_initializer())
            self.W3 = tf.get_variable("W3_2", [20, 1], initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b3 = tf.get_variable("b3_2", [1], initializer=tf.zeros_initializer())
...
checkpoint_path = "C:/Users/guyz/Documents/data/drl3-transfer/models/mountain.ckpt"
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            if done:
                ...
                selected_velocity.value.add(tag='Velocity selected', simple_value=np.mean(velocity_container))
                tb_summary_writer.add_summary(selected_velocity, episode)
                if episode > 98:
                    # Check if solved
